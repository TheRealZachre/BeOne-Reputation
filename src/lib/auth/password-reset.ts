...
async function writeResetDb(db: PasswordResetDatabase): Promise<void> {
  ...
  db.tokens = db.tokens.filter(
    (entry) =>
      !entry.usedAt &&
      new Date(entry.expiresAt).getTime() > now - 24 * 60 * 60 * 1000
  );
...
export function buildPasswordResetUrl(origin: string, token: string): string {
  return `${origin.replace(/\/$/, "")}/reset-password?token=${encodeURIComponent(
    token
  )}`;
}
...
export async function sendPasswordResetEmail(
  ...
  return sendResendEmail({
    to: email,
    subject: "Reset your BeOne Reputation password",
    html: `<p>You requested a password reset for your BeOne Reputation account.</p><p><a href="${resetUrl}">Reset your password</a></p><p>This link expires in 1 hour. If you did not request this, you can safely ignore this email.</p>`,
  });
}
